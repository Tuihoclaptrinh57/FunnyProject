...
export type Comment = {
  ...
  createdAt: string;
  resolved: boolean;
  // range for demo (not real CRDT position)
...
export function useCollab(initialTitle = 'Untitled document', initialContent = '') {
  ...
  const [comments, setComments] = useState<Comment[]>([
    { id: genId(), author: 'client-ax3', content: 'Đoạn này nên tách thành 2 câu cho rõ hơn.', quote: 'không xung đột', createdAt: '2m ago', resolved: false },
    { id: genId(), author: clientId, content: 'Mình đã sửa theo góp ý.', quote: 'tự đồng bộ khi có mạng', createdAt: '1m ago', resolved: false },
  ]);
  ...
  // Simulate CRDT/WebSocket connection states
  useEffect(() => {
    const t1 = setTimeout(() => setConnection('connected'), 900);
    // toggle offline occasionally for demo (or allow manual toggle)
    return () => clearTimeout(t1);
  }, []);

  // pending increments when offline + typing
  const handleContentChange = (next: string) => {
    if (connection === 'offline') setPending((p) => p + 1);
    else if (pending > 0) {
      // simulate flushing when back online
      setTimeout(() => setPending(0), 600);
    }
    pushUndo(content);
    setContent(next);
  };
  ...
  // selection for new comment (updated from editor)
  const [selection, setSelection] = useState<{ text: string; range: { start: number; end: number } } | null>(null);

  return {
    clientId,
    title, setTitle,
    content, handleContentChange, setContent,
    connection, pending, setOffline, setConnected,
    comments, addComment, toggleResolve, deleteComment,
